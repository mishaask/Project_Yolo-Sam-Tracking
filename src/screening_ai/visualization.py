...
def draw_detection(
    ...
    label = f"{class_name} G{track_id}{local_text} {confidence:.2f}{owner_text}{reid_text}"
    ...
    if mask is not None:
        ...
        overlay = frame.copy()
        overlay[mask > 0] = (0.5 * overlay[mask > 0] + 0.5 * np.array([0, 220, 220])).astype(np.uint8)
...
def draw_owner_links(
    frame: np.ndarray,
    links: list[tuple[tuple[int, int], tuple[int, int], int, int]],
) -> None:
    for bag_point, owner_point, bag_id, owner_id in links:
        cv2.line(frame, bag_point, owner_point, (255, 0, 255), 2)
        mid = ((bag_point[0] + owner_point[0]) // 2, (bag_point[1] + owner_point[1]) // 2)
        cv2.putText(
            frame,
            f"G{bag_id}->G{owner_id}",
            mid,
            cv2.FONT_HERSHEY_SIMPLEX,
            0.45,
            (255, 0, 255),
